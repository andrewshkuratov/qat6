...
public class DatabaseService {
    ...
    private static final String GET_BY_OWNER_SQL =
            "select * from databases where owner = :owner";
    private static final String DB_SIZE =
            "SELECT pg_database_size(:database)";
    private static final ParameterizedStringFactory CREATE_DATABASE_SQL =
            new ParameterizedStringFactory(
                    "CREATE database :name WITH OWNER :owner TABLESPACE :tablespace");
    private static final String REGISTER_DATABASE_SQL =
            "insert into databases(name, owner) values(:name, :owner)";
    ...
    */
    ...
    public static Database byName(String dbName) {
        var database = DBPool.getConnection()
                .open()
                .createQuery(GET_BY_NAME_SQL)
                .addParameter("name", dbName)
                .executeAndFetchFirst(Database.class);
        if (database == null) {
            throw new NotFoundException(String
                    .format("Database with name %s not found", dbName));
        }
    ...
    public static List<Database> forUser(String owner) {
        return DBPool.getConnection().open()
                .createQuery(GET_BY_OWNER_SQL)
    ...
    public static void create(String dbName, String owner, String tablespace) {
        ...
        String query = CREATE_DATABASE_SQL
                .addParameter("name", dbName)
                .addParameter("tablespace", tablespace)
                .addParameter("owner", owner).toString();
    ...
    private static final String UNREGISTER_DATABASE_SQL =
            "delete from databases where name=:name and owner= :owner";

    public static void drop(Database database) {
        var connection = DBPool.getConnection().open();
        for (Backup point : BackupService.list(database.getName())) {
            BackupService.delete(database.getOwner(),
                    point.getDatabase(), point.getPoint());
        }
        for (Script script : ScriptService.list(database.getName())) {
           ScriptService.destroy(script);
        }
        String query = DROP_DATABASE_SQL
                .addParameter("name", database.getName()).toString();
        connection.createQuery(query, false)
                .executeUpdate();
