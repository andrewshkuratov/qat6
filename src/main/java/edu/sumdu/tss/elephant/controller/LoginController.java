...
public class LoginController extends AbstractController {
    ...
    public static void create(Context context) {
        ...
        try {
            var login = context.formParamAsClass("login", String.class)
                    ...
                    .check(ValidatorHelper::isValidMail,
                            mb.get("validation.mail.invalid"))
                    .get();
            password = context.formParamAsClass("password", String.class)
                    .check(it -> it != null && !it.isBlank(),
                            mb.get("validation.password.empty"))
                    .get();
            user = UserService.byLogin(login);
        } catch (NotFoundException ex) {
            context.sessionAttribute(Keys.ERROR_KEY,
                    mb.get("validation.user.not_found"));
        } catch (Exception ex) {
            ExceptionUtils.wrapError(context, ex);
        }
        if (user != null && user.getPassword()
                .equals(user.crypt(password))) {
    ...
    public static void resetLink(Context context) {
        ...
        try {
            if (context.method().equals("POST")) {
                ...
                MailService.sendResetLink(user.getToken(),
                        user.getLogin(), lang);
                context.sessionAttribute(Keys.INFO_KEY,
                        mb.get("login.reset.success_send"));
                context.redirect(BASIC_PAGE, 302);
                return;
            }
        ...
        model.put(Keys.ERROR_KEY,
                context.sessionAttribute(Keys.ERROR_KEY));
        model.put(Keys.INFO_KEY,
                context.sessionAttribute(Keys.INFO_KEY));
        ...
        try {
            if (context.method().equals("POST")) {
                String token = context
                        .formParamAsClass("token", String.class)
                        .check(it -> it != null && !it.isBlank(),
                                mb.get("validation.token.empty"))
                        .get();
                var password = context
                        .formParamAsClass("password", String.class)
                        .check(it -> it != null && !it.isBlank(),
                                mb.get("validation.password.empty"))
                        .check(ValidatorHelper::isValidPassword,
                                mb.get("validation.invalid.empty"))
                        .get();
                ...
                context.sessionAttribute(Keys.INFO_KEY,
                        mb.get("login.reset.success_reset"));
                context.redirect(BASIC_PAGE);
                return;
            }
        } catch (NotFoundException ex) {
            context.sessionAttribute(Keys.ERROR_KEY,
                    mb.get("validation.user.not_found_token"));
            context.redirect(BASIC_PAGE);
        } catch (Exception ex) {
            ExceptionUtils.wrapError(context, ex);
        }
    ...
    public static void lang(Context context) {
        ...
        context.sessionAttribute(Keys.LANG_KEY,
                Optional.ofNullable(lang)
                        .orElse(Keys.get("DEFAULT_LANG")));
        ViewHelper.redirectBack(context);
    }

    public void register(Javalin app) {
        app.get(BASIC_PAGE, LoginController::show,
                UserRole.ANYONE);
        app.post(BASIC_PAGE, LoginController::create,
                UserRole.ANYONE);
        app.get(BASIC_PAGE + "/reset-password",
                LoginController::resetLink, UserRole.ANYONE);
        app.post(BASIC_PAGE + "/reset-password",
                LoginController::resetLink, UserRole.ANYONE);
        app.get(BASIC_PAGE + "/reset",
                LoginController::resetPassword, UserRole.ANYONE);
        app.post(BASIC_PAGE + "/reset",
                LoginController::resetPassword, UserRole.ANYONE);
        app.get(BASIC_PAGE + "/lang/{lang}",
                LoginController::lang, UserRole.ANYONE);
        app.get("/logout",
                LoginController::destroy, UserRole.AUTHED);
    }
